...
async def story_writer_start(callback_context: CallbackContext) -> types.Content | None:
    callback_context.state["temp:story_writer_status"] = "스토리 작성 중..."
    return None


async def story_writer_progress(callback_context: CallbackContext) -> types.Content | None:
    ...
    return types.Content(
        ...
        parts=[
            types.Part.from_text(
                text=(
                    f"스토리 작성 완료: '{story_book.title}' ({story_book.age_range} 대상, "
                    f"총 {len(story_book.pages)}페이지). 교훈: {story_book.moral}"
                )
            )
        ],
    )
...
사용자가 입력한 원래 주제는 `theme` 필드에 그대로 반영하세요.
    ...
    before_agent_callback=story_writer_start,
    after_agent_callback=story_writer_progress,
)
